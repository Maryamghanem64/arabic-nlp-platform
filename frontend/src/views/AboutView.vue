<template>
  <div class="page-wrap reports-page page-stack">
    ...
    <section class="panel panel-pad">
      <div class="section-head"><div><h2 class="section-title">Analyzer taxonomy</h2><p class="section-subtitle">Tools are grouped by research role, not by visual convenience.</p></div></div>
      <div class="taxonomy-grid">
        <article v-for="group in taxonomy" :key="group.name" class="taxonomy-card"><strong>{{ group.name }}</strong><span>{{ group.tools }}</span><p>{{ group.role }}</p></article>
      </div>
    </section>
...
const sections = [
  ...
  { kicker:'03 · Fusion', title:'Capability-based expert fusion', text:'Feature selection is routed by analyzer strength rather than blind voting.', items:['Farasa anchors segmentation evidence.','CAMeL and SinaTools contribute lexical/morphological evidence when available.','Stanza and UDPipe contribute UD-oriented syntax evidence.','The selected source and conflicts remain visible.'] },
  { kicker:'04 · Evaluation', title:'Capability-aware agreement', text:'Unsupported fields are excluded from feature denominators.', items:['POS agreement is restricted to eligible POS analyzers.','Lemma indicators distinguish normalized and exact matching.','Segmentation coverage reports evidence availability.','No metric is presented as gold-standard accuracy.'] },
...
const taxonomy = [
  { name:'Morphology / lexical evidence', tools:'CAMeL · SinaTools · AlKhalil · MADAMIRA (optional)', role:'Lemma, root, POS, and morphological evidence according to tool capability.' },
  ...
  { name:'Contextual representation', tools:'AraBERT', role:'Contextual embeddings; not a direct morphology-table competitor.' },
]
</script>

<style scoped>
.compact-hero{padding:34px 38px}.reports-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.report-card p,.boundary-panel p,.taxonomy-card p{color:var(--c-text-secondary);line-height:1.7}.section-kicker{display:block;margin-bottom:8px;color:var(--c-accent-text);font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase}.report-list{margin:14px 0 0;padding-inline-start:18px;color:var(--c-text-secondary)}.report-list li+li{margin-top:7px}.taxonomy-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.taxonomy-card{padding:15px;border:1px solid var(--c-border);border-radius:10px;background:var(--c-page-bg)}.taxonomy-card{display:grid;gap:6px}.taxonomy-card span{color:var(--c-accent-text);font-size:13px}.taxonomy-card p{margin:0;font-size:13px}.boundary-panel{border-left:4px solid var(--c-accent)}@media(max-width:900px){.reports-grid,.taxonomy-grid{grid-template-columns:1fr}}
</style>
